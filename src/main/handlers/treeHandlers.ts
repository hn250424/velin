...
export default function registerTreeHandlers(
	...
	ipcMain.handle(
		...
		async (e, prePath: string, newPath: string) => {
	...
	ipcMain.handle(
		...
		async (e, src: string, dest: string) => {
	...
	ipcMain.handle(
		electronAPI.events.rendererToMain.delete,
		async (e, arr: string[]) => {
	...
	ipcMain.handle(
		...
		async (e, trashMap: TrashMap[] | null) => {
	...
	ipcMain.handle(
		...
		async (e, path: string, directory: boolean) => {
