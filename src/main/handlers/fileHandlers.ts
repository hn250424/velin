...
export default function registerFileHandlers(
	...
	ipcMain.handle(
		electronAPI.events.rendererToMain.save,
		async (e, data: TabEditorDto) => {
			const tabEditorData: TabEditorDto = await fileService.save(
				data,
				mainWindow
			);
			return {
	...
	ipcMain.handle(
		electronAPI.events.rendererToMain.saveAs,
		async (e, data: TabEditorDto) => {
			const tabEditorData: TabEditorDto = await fileService.saveAs(
				data,
				mainWindow
			);
			return {
	...
	ipcMain.handle(
		electronAPI.events.rendererToMain.saveAll,
		async (e, data: TabEditorsDto) => {
			const dataArr: TabEditorsDto = await fileService.saveAll(
				data,
				mainWindow
			);
			return {
