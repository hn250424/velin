...
window.addEventListener("DOMContentLoaded", () => {
	...
	const findReplaceState = diContainer.get<FindReplaceState>(
		DI_KEYS.FindReplaceState
	);
	...
	const shortcutRegistry = diContainer.get<ShortcutRegistry>(
		DI_KEYS.ShortcutRegistry
	);

	const settingsFacade = diContainer.get<SettingsFacade>(
		DI_KEYS.SettingsFacade
	);
	const tabEditorFacade = diContainer.get<TabEditorFacade>(
		DI_KEYS.TabEditorFacade
	);
	const treeFacade = diContainer.get<TreeFacade>(DI_KEYS.TreeFacade);

	const commandDispatcher = diContainer.get<CommandDispatcher>(
		DI_KEYS.CommandDispatcher
	);
...
function bindSyncEventFromWatch(
	tabEditorFacade: TabEditorFacade,
	treeFacade: TreeFacade
) {
	window.mainToRenderer.syncFromWatch(
		async (tabEditorsDto: TabEditorsDto, treeDto: TreeDto) => {
			if (tabEditorsDto) {
...
function bindShortcutEvent(
	...
	shortcutRegistry.register(
		"ESC",
		async (e: KeyboardEvent) => await commandDispatcher.performESC("shortcut")
	);
	shortcutRegistry.register(
		"ENTER",
		async (e: KeyboardEvent) =>
			await commandDispatcher.performENTER(e, "shortcut")
	);
}
